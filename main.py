...
def captureSmall():
    ...
    data = dev.ctrl_transfer(0xc0, 0x4e, 2, 0, 3)           #returns 4e0100
    data = dev.ctrl_transfer(0xc0, 0x4e, 3, 0, 3)           #returns 4e0100
    data = dev.ctrl_transfer(0xc0, 0x46, 0x7b7, 2, 3)       #returns 460100
    data = dev.ctrl_transfer(0xc0, 0x47, 0x10, 2, 3)        #returns 470100
    data = dev.ctrl_transfer(0xc0, 0x49, 0x100, 2, 3)       #returns 490100
    data = dev.ctrl_transfer(0xc0, 0x4c, 0xc0, 96, 5)       #returns 4c0100f000 - image height?
    data = dev.ctrl_transfer(0xc0, 0x46, 0xc8, 3, 3)        #returns 460100
    data = dev.ctrl_transfer(0xc0, 0x47, 0x10, 3, 3)        #returns 470100
    data = dev.ctrl_transfer(0xc0, 0x49, 0x100, 3, 3)       #returns 490100
    data = dev.ctrl_transfer(0xc0, 0x42, 0, 258, 3)         #returns 420100
    data = dev.ctrl_transfer(0xc0, 0x43, 0, 0, 3)           #returns 430100
    data = dev.ctrl_transfer(0xc0, 0x4c, 0xc0, 96, 5)       #returns 4c0100f000 - image height?
    data = dev.ctrl_transfer(0xc0, 0x44, 2, 0, 6)           #returns 440100f00000

    print("Capture 4")
    dat = bulk_receive(2,61440)                             #vein data
    for i in range(96 * 640):
        dat[i] ^= mask[i]
    res.append(bufToImage(dat, 640, 96))
    
    print("Capture 5")
    data = dev.ctrl_transfer(0xc0, 0x44, 2, 1, 6)           #returns 440100f00000
    dat = bulk_receive(2,61440)                             #normal picture
    res.append(bufToImage(dat, 640, 96))
